...
const VEHICLE_LABELS: Record<string, { label: string; tier: string }> = {
  SEDAN: { label: "Sedan", tier: "Comfort" },
  SUV: { label: "SUV", tier: "Comfort" },
  VAN: { label: "Van", tier: "Group" },
  MINIBUS: { label: "Minibus", tier: "Group" },
  COACH: { label: "Coach", tier: "Large group" },
};
        ...
        <>
          ...
          <div className="space-y-3">
            ...
            {route.vehicles.map((v) => {
              const meta = VEHICLE_LABELS[v.vehicleType] ?? { label: v.vehicleType, tier: "" };
              return (
                <div key={v.rateId} className="bg-white border border-slate-200 rounded-xl p-4 hover:border-trivia-300 hover:shadow-soft transition-all">
                  <div className="flex flex-col md:flex-row gap-4 md:items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {meta.tier && (
                          <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-trivia-50 text-trivia-700">{meta.tier}</span>
                        )}
                      </div>
                      <div className="flex flex-wrap gap-3 mt-2 text-xs text-slate-600">
                        <span className="flex items-center gap-1">
                          <Users className="w-3 h-3 text-slate-400" /> Up to {v.maxPax}
                        </span>
                        {v.maxLuggage != null && (
                          <span className="flex items-center gap-1">
                            <Briefcase className="w-3 h-3 text-slate-400" /> {v.maxLuggage} bags
                          </span>
                        )}
                      </div>
                    ...
                    <div className="md:w-44 flex flex-col items-stretch md:items-end justify-between border-t md:border-t-0 md:border-l border-slate-100 pt-3 md:pt-0 md:pl-4">
                      ...
                      <Link
                        href={`/transfers/book?rateId=${v.rateId}&routeId=${route.routeId}&${searchQuery}`}
